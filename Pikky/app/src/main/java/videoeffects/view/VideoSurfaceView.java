...
public class VideoSurfaceView extends GLSurfaceView {
    ...
    public void init(MediaPlayer mediaPlayer, ShaderInterface shaderEffect) {
        if (mediaPlayer == null)
            Toast.makeText(mContext, "Set MediaPlayer before continuing", Toast.LENGTH_LONG).show();
        else
    ...
    private static class VideoRender implements Renderer, SurfaceTexture.OnFrameAvailableListener {
        ...
        // code for rotating the video for the device which has video inverted
        private final float[] mTriangleVerticesData8 = {
                // X, Y, Z, U, V
                -1.0f, -1.0f, 0, 1.f, 0.f,
                1.0f, -1.0f, 0, 1.f, 1.f,
                -1.0f,  1.0f, 0, 0.f, 0.f,
        ...
        public VideoRender(Context context) {
            ...
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.O) {
                mTriangleVertices = ByteBuffer
                        .allocateDirect(
                                mTriangleVerticesData8.length * FLOAT_SIZE_BYTES)
                        .order(ByteOrder.nativeOrder()).asFloatBuffer();
                mTriangleVertices.put(mTriangleVerticesData8).position(0);
            }else{
                mTriangleVertices = ByteBuffer
                        .allocateDirect(
        ...
        public void onDrawFrame(GL10 glUnused) {
            ...
            /*
            mSurfaceView.getLayoutParams().height = mSurfaceView.getMeasuredWidth();
            mSurfaceView.setMeasuredDimension(640, 640);
            Log.i(Configurations.TAG, "SURFACE HEIGHT: " + mSurfaceView.getHeight());
            */
        ...
        private void checkGlError(String op) {
            ...
            while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
                Log.i(Configurations.TAG, op + ": glError " + error);
